            ...
            <div class="form-group text-dark">
              <div class="form-group">
                ...
                <select name="class_id" id="" class="form-control select2 select2-sm" required>
                  <option></option> 
                  <?php 
                        $classes = $conn->query("SELECT * FROM classes order by degree asc,section asc ");
                        while($row = $classes->fetch_array()):
                  ?>
                        <option value="<?php echo $row['id'] ?>" <?php echo isset($class_id) && $class_id == $row['id'] ? "selected" : '' ?>><?php echo ucwords($row['degree'].'-'.$row['section']) ?></option>
                  <?php endwhile; ?>
                </select>
              </div>
            </div>
